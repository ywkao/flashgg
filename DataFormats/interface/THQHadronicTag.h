#ifndef FLASHgg_THQHadronicTag_h
#define FLASHgg_THQHadronicTag_h
...
namespace flashgg {
    ...
    class THQHadronicTag: public DiPhotonTagBase
    {
    public:
        THQHadronicTag();
        THQHadronicTag( edm::Ptr<DiPhotonCandidate>, edm::Ptr<DiPhotonMVAResult> );
        THQHadronicTag( edm::Ptr<DiPhotonCandidate>, DiPhotonMVAResult );

        ~THQHadronicTag();

        THQHadronicTag *clone() const { return ( new THQHadronicTag( *this ) ); }
        ...
        const std::vector<int > get_mom_pdgId() const { return mom_pdgId_;}
        const std::vector<int > get_mom_status() const { return mom_status_;}
        const std::vector<double > get_mom_pt() const { return mom_pt_;}
        const std::vector<double > get_mom_eta() const { return mom_eta_;}
        const std::vector<double > get_mom_phi() const { return mom_phi_;}
...
//
        ...
        void setMomPdgId( std::vector<int > mom_pdgId ) { mom_pdgId_ = mom_pdgId;}
        void setMomStatus( std::vector<int > mom_status ) { mom_status_ = mom_status;}
        void setMomPt( std::vector<double > mom_pt ) { mom_pt_ = mom_pt;}
        void setMomEta( std::vector<double > mom_eta ) { mom_eta_ = mom_eta;}
        void setMomPhi( std::vector<double > mom_phi ) { mom_phi_ = mom_phi;}
        void setMomMass( std::vector<double > mom_mass ) { mom_mass_ = mom_mass;}
...
//
    ...
    private:
        ...
        std::vector<int > mom_pdgId_;
        std::vector<int > mom_status_;
        std::vector<double > mom_pt_;
        std::vector<double > mom_eta_;
        std::vector<double > mom_phi_;
        std::vector<double > mom_mass_;
